...
export const AuthByUsername = (props: AuthByUsernameProps) => {
    const {className, onChangeLogin, onChangePassword, onChangeRepeatPassword} = props;
    const location = useLocation();
    const isLogin = location.pathname === AppRoutes.LOGIN

    return (
            <form className={classNames(cls.AuthByUsername, {}, [className])}>
                {isLogin ? <Title subTitle={'Авторизация'} color='white' align='center'></Title> : <Title subTitle={'Регистрация'} color='white' align='center'></Title>}
                <div className={classNames(cls.form, {}, [className])}>
                    <Input onChange={onChangeLogin} type='text' placeholder='Введите логин' />
                    <Input onChange={onChangePassword} type='text' placeholder='Введите пароль' />
                    {!isLogin && <Input onChange={onChangeRepeatPassword} type='text' placeholder='Повторите пароль' />}
                </div>
